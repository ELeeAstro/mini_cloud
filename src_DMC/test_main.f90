program test_DMC
  ...
  use mini_cloud_DMC_mod, only : mini_cloud_DMC, rho_d, mol_w_sp
  ...
  time = 6840.0_dp
    ...
    ! Find number of lines in file
      ...
      do i = 1, nlay
        print*, i, pl(i)/1e5_dp, Tl(i), Kzz(i), q_v(i), q_c(i)
      end do
...
contains
  ...
  subroutine output(t, time, nlay)
    ...
    if (first_call .eqv. .True.) then
      open(newunit=u1,file='results_DMC/tracers.txt',action='readwrite')
      open(newunit=u2,file='results_DMC/opac.txt',action='readwrite')
      write(u2,*) wl(:)
      first_call = .False.
    end if
