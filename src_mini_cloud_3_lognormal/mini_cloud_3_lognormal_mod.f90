module mini_cloud_3_lognormal_mod
  ...
  real(dp) function p_vap_sp(sp, T)
    ...
    case('MgO')
      ...
      p_vap_sp = exp(-8.44591675e4_dp/T + 4.58952742e1_dp + 4.38797163e-3_dp*T - 9.84437451e-8_dp*T**2 - 7.30855823e-11_dp*T**3)
    ...
    case('H2O')
      ...
      if (T <= 273.16_dp) then
        p_vap_sp = exp(9.550426_dp - 5723.265_dp/T + 3.53068_dp*log(T) - 0.00728332_dp*T) * pa
      else if (T < 1048.0_dp) then
        p_vap_sp = exp(54.842763_dp - 6763.22_dp/T - 4.210_dp*log(T) + 0.000367_dp*T &
          & + tanh(0.0415_dp*(T - 218.8_dp)) &
          & * (53.878_dp - 1331.22_dp/T - 9.44523_dp*log(T) + 0.014025_dp*T)) * pa
      else
        p_vap_sp = exp(54.842763_dp - 6763.22_dp/1048.0_dp - 4.210_dp*log(1048.0_dp) + 0.000367_dp*1048.0_dp &
    ...
    case('NH3')
      ...
      ! Fray & Schmitt (2009)
      p_vap_sp = exp(15.96_dp - 3537.0_dp/T - 3.310e4_dp/T**2 + 1.742e6_dp/T**3 - 2.995e7_dp/T**4) * bar
    case('CH4')
      ! Frey & Schmitt (2009)
