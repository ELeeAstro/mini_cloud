module mini_cloud_2_mod
  ...
  subroutine RHS_mom(n_eq, time, y, f)
    ...
    Kn_p = beta/(6.0_dp*eta*r_c**2) * sqrt((m_c*kb*T)/(2.0_dp*pi))
  ...
  subroutine calc_cond(n_eq, y, sat, r_c, rho_s, Kn, dmdt)
    implicit none

    integer, intent(in) :: n_eq
    real(dp), dimension(n_eq), intent(in) :: y 
    real(dp), intent(in) :: sat, r_c, rho_s, Kn
  ...
  subroutine calc_coag(n_eq, y, m_c, r_c, beta, Kn_p, f_coag)
    ...
    integer, intent(in) :: n_eq
    real(dp), dimension(n_eq), intent(in) :: y 
    real(dp), intent(in) :: m_c, r_c, beta, Kn_p
