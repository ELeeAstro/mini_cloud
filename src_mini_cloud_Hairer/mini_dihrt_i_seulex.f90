module mini_dihrt_i_seulex
  ...
  public ::  mini_dihrt_seulex, RHS_update, jac_dummy, mas_dummy, solout

contains

  subroutine mini_dihrt_seulex(n_dust, T_in, P_in, t_end, sp, k, k3, VMR)
    implicit none
    ...
    if (k(1) > 1e-10_dp .and. &
      & (a_check <= a_seed*1.1_dp .or. Ar_check <= Ar_seed*1.1_dp .or. V_check <= V_seed*1.1_dp)) then
      k(1) = k(1)
      k(2) = a_seed * k(1)
    ...
    ! If overall growth is super slow, don't bother integrating
     if (k(1) > 1.0e-10_dp .and. (sum(d_sp(:)%Js) < 1.0e-10_dp &
       & .and. abs(sum(d_sp(:)%chis)) < 1.0e-10_dp)) then
         return
    end if
    ...
    do while((t_now < t_end))
      ...
      call SEULEX(neq,RHS_update,0,t_now,y,t_end,dt_init, &
        &                  rtol,atol,itol, &
  ...
  end subroutine mini_dihrt_seulex
...
end module mini_dihrt_i_seulex
