module mini_cloud_2_mod
  ...
  subroutine mini_cloud_2(T_in, P_in, grav_in, mu_in, bg_VMR_in, t_end, sp, sp_bg, q_vs, q_0, q_1s)
    implicit none

    ! Input variables
    character(len=20), dimension(:), intent(in) :: sp, sp_bg
    ...
    real(dp), dimension(:), intent(inout) :: q_vs, q_1s
  ...
  subroutine RHS_mom(n_eq, time, y, f)
    ...
    f(1) = (f_nuc_hom + f_nuc_het + f_seed_evap) + f_coag + f_coal + f_ti + f_tsh
  ...
  subroutine calc_turb_inert(n_eq, y, r_c, beta, f_ti)
    ...
    !! Turbulent inertial collision rate
  ...
  subroutine calc_turb_shear(n_eq, y, r_c, f_tsh)
    ...
    !! Turbulent shear collision rate
