module mini_cloud_3_lognormal_mix_mod
  ...
  !! Conversions to dyne
  real(dp), parameter :: bar = 1.0e6_dp
  real(dp), parameter :: atm = 1.01325e6_dp
  real(dp), parameter :: pa = 10.0_dp
  real(dp), parameter :: mmHg = 1333.22387415_dp
  ...
  !! Arrays for background gas mixture viscosity
  ...
  private :: calc_coal, calc_coag, calc_cond, calc_hom_nuc, calc_seed_evap, &
    & p_vap_sp, sig_sp, l_heat_sp, eta_a_mix, kappa_a_mix, cloud_sp

  contains

  subroutine mini_cloud_3_lognormal_mix(ilay, T_in, P_in, grav_in, mu_in, met_in, cp_in, &
      & bg_VMR_in, t_end, sp_in, sp_bg, n_in, q_v, q_0, q_1, q_2, dTdt)
    implicit none
  ...
  subroutine RHS_mom(n_eq, time, y, f)
    ...
    vf_s = (2.0_dp * beta * grav * r_c**2 * (rho_d_m - rho))/(9.0_dp * eta_a) &
      & * (1.0_dp + ((0.45_dp*grav*r_c**3*rho*rho_d_m)/(54.0_dp*eta_a**2))**(0.4_dp))**(-1.25_dp)
  ...
  subroutine calc_cond(ndust, r_med, Kn, Kn_m, Kn_m2, n_v, V_mix, lnsig2, dmdt1, dmdt2)
    ...
    real(dp), intent(in) :: r_med, Kn, Kn_m, Kn_m2, lnsig2
    ...
    do j = 1, ndust
      ...
      c_facl1 = 4.0_dp * pi * r_med * cld(j)%D * cld(j)%m0 * n_v(j) * (1.0_dp - 1.0_dp/cld(j)%sat)
      dmdt_low1 = c_facl1 * exp(1.0_dp/18.0_dp * lnsig2)
      dmdt_low2 = c_facl1 * exp(7.0_dp/18.0_dp * lnsig2)

      c_facg1 = 4.0_dp * pi * r_med**2 * cld(j)%vth * cld(j)%m0 * n_v(j) * cld(j)%alp_c * &
  ...
  subroutine calc_hom_nuc(ndust, n_v, J_hom)
    ...
    do j = 1, ndust
      ...
      if (cld(j)%sat > 1.0_dp) then
        ...
        if (J_hom(j) < 1e-10_dp) J_hom(j) = 0.0_dp
  ...
  subroutine calc_coag(m_med, r_med, lnsig2, Kn_in, f_coag0, f_coag2)
    implicit none

    real(dp), intent(in) :: m_med, r_med, lnsig2, Kn_in

    real(dp), intent(out) :: f_coag0, f_coag2

    real(dp) :: Knd0, phi0, Kl0, Kh0, exp_fac_l_0, exp_fac_h_0
    ...
    exp_fac_h_0 = H * (exp(25.0_dp/72.0_dp * lnsig2) + 2.0_dp*exp(5.0_dp/72.0_dp * lnsig2) + exp(1.0_dp/72.0_dp * lnsig2))
    exp_fac_h_2 = H * (exp(37.0_dp/72.0_dp * lnsig2) + 2.0_dp*exp(17.0_dp/72.0_dp * lnsig2) + exp(13.0_dp/72.0_dp * lnsig2))

    Kl0 = (4.0_dp*kb*T)/(3.0_dp*eta_a) * exp_fac_l_0
    Kl2 = (4.0_dp*kb*T)/(3.0_dp*eta_a) * exp_fac_l_2

    Kh0 = 2.0_dp * sqrt((8.0_dp*pi*kb*T)/m_med) * r_med**2 * exp_fac_h_0
    Kh2 = 2.0_dp * sqrt((8.0_dp*pi*kb*T)/m_med) * r_med**2 * exp_fac_h_2
  ...
  subroutine kappa_a_mix(n_bg, sp_bg, VMR_bg, T, kappa_out)
    ...
    do n = 1, n_bg
      ...
      case('N2')
        Tr = T/126.192_dp
        lam_g(n) = (-50.4059_dp + 1457.09_dp*Tr - 2580.5_dp*Tr**2 + 3616.59_dp*Tr**3 &
          & - 695.367_dp*Tr**4 + 129.796_dp*Tr**5 + 4.03114_dp*Tr**6) &
          & / (99.3852_dp - 151.972_dp*Tr + 229.102_dp*Tr**2 - 22.9451_dp*Tr**3 + 6.0789_dp*Tr**4 + Tr**5)
        lam_g(n) = lam_g(n) * 1e4_dp * 1e-2_dp

      case('O2')
        lam_g(n) = -3169.0_dp + 735.7_dp*sqrt(T) - 53.83_dp*T &
          & + 2.583_dp*T**1.5 - 0.05325_dp*T**2 + 4.083e-4_dp*T**2.5

      case('NH3')
        Tr = T/405.56_dp
        lam_g(n) = (86.9294_dp - 170.5502_dp*Tr + 608.0287_dp*Tr**2 &
          & - 100.9764_dp*Tr**3 + 85.1986_dp*T**4) &
          & / (4.68994_dp + 9.21307_dp*Tr - 1.53637_dp*Tr**2 + Tr**3)
        lam_g(n) = lam_g(n) * 1e4_dp * 1e-2_dp

      case('CO2')
        Tr = T/304.1282_dp
        lam_g(n) = sqrt(Tr)/(1.51874307e-2_dp + 2.80674040e-2_dp/Tr &
          & + 2.28564190e-2_dp/Tr**2 - 7.41624210e-3_dp/Tr**3)
        lam_g(n) = lam_g(n) * 1e4_dp * 1e-2_dp

      case('CO')
        lam_g(n) = 25.0_dp * 1e4_dp * 1e-2_dp

      case('CH4')
        lam_g(n) = 34.4_dp * 1e4_dp * 1e-2_dp

      case('C2H2')
        lam_g(n) = 21.4_dp * 1e4_dp * 1e-2_dp
    ...
    do i = 1, n_bg
      kappa_sum = 0.0_dp
      if ((VMR_bg(i) < 1e-20_dp) .or. (lam_g(i) < 1e-20_dp)) cycle
      do j = 1, n_bg
        if (VMR_bg(j) < 1e-20_dp) cycle
